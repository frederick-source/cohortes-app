...
# ============================================================
# PROCESAMIENTO E INICIALIZACIÓN DE VARIABLES CRÍTICAS
# ============================================================
# Inicializamos variables de YOM para que el reporte no falle si no se entra al Tab 4
df_resumen_nuevos = None
df_resumen_rec = None
total_monthly_grand_impact = 0

file_bytes = uploaded_file.getvalue()
try:
    df_raw_load = backend.load_csv(file_bytes)
    # Limpieza simple de nombres de columnas para facilitar la selección
...
k2.metric("Base Activa (Recurrentes)", backend.fmt_int(avg_rec_active), help="Clientes recurrentes (>12 meses) que han comprado en los últimos 6 meses.")
...
with tab2:
    ...
    **¿Qué estamos viendo?**
    ...
    m1, m2, m3 = st.columns(3)
    m1.metric("Base Activa (6m)", backend.fmt_int(avg_rec_active), help="Clientes recurrentes con compra en los últimos 6 meses.")
...
with tab4:
    ...
    acumulado_clientes_res_val = clientes_resucitados_mes * 12
    ...
    acumulado_dinero_res = matriz_res_impacto.sum() 

    # --- RESURRECCIÓN (Proyección años atrás) ---
    ...
    # --- RECURRENTES ---
    base_rec_actual = avg_rec_active if pd.notnull(avg_rec_active) else 0 
...
# ============================================================
# DESCARGA DEL REPORTE (HTML)
# ============================================================
st.markdown("---")
if uploaded_file is not None:
    
    # Recalculamos las tablas para el reporte para asegurar que existen 
    # (incluso si el usuario no ha entrado a los Tabs)
    ids_new_viz = cohort_df[cohort_df["edad_actual_meses"].between(0, 11)].index
    t_new_report, _, _ = backend.tabla_por_mesvida(ids_new_viz, range(0, 12), cm_arpu)

    ids_rec_viz = cohort_df[cohort_df["edad_actual_meses"].between(12, 37)].index
    t_rec_report, _, _ = backend.tabla_por_mesvida(ids_rec_viz, range(12, 38), cm_arpu)

    # Generamos el HTML usando la nueva función del backend
    html_content = backend.build_full_report_html(
        ids_nuevos_len=len(ids_nuevos), 
        ids_recurrentes_len=len(ids_recurrentes),
        outlier_customers_len=len(outlier_customers), 
        limite_outlier=limite_outlier,
        avg_new_entries=avg_new_entries, 
        avg_rec_active=avg_rec_active,
        t_new=t_new_report, 
        t_rec=t_rec_report, 
        ret_table=ret_table,
        # PASAMOS LOS RESULTADOS DE YOM (Si existen, si no, irán como None/0)
        df_yom_new=df_resumen_nuevos,
        df_yom_rec=df_resumen_rec,
        yom_impacto_total=fmt_clp(total_monthly_grand_impact)
    )

    st.download_button(
        label="📥 Descargar Reporte HTML",
        data=html_content.encode("utf-8"),
        file_name=f"Flux_Reporte_{datetime.now().strftime('%Y%m%d')}.html",
        mime="text/html"
    )
